...
{
    public class Emera3Driver : Sys.Types.Components.DriverElectroClient
    {
        #region (FineTune)
        static FineTune FineTune;
        static Emera3Driver()
        {
            FineTune = FineTune.TryLoad("emera3electro");
        }

        public Int32 ReadTimeOutRequestMSec()
        {
            return FineTune.ReadValue<int>("TimeOutRequestMSec", this, (v) => Int32.Parse(v), Emera3Request.TimeOutRequestMSecDeafult);
        }
        #endregion

        /// <summary>Идентификатор типа реализации компонента</summary>
        public const string TypeGuidImpl = "est.by:Bus.Emera3DrvClientImpl";
        ...
        public override SynchResponse Synch(SynchRequestDataDrv requestData, SynchParamsDataDrv requestParam)
        {
            ...
            var info = new QueryInfo(
                ...
                requestData,
                requestParam,
                requestData.GetSharedSetting(() => new Emera3SharedSetting()),
                requestData.GetContentSetting(() => new Emera3ContentSetting()),
                requestData.GetDriverSetting(() => new DriverSetting())
            );

            //Любая работа с устройством должна начинаться с открытия сессии
            using (var session = new PhysicalSession<Emera3SynchState, AccountNextPoint>(this, requestData, info.NextPoint))
            {
                ...
                if (session.LaunchPoint(info.NextPoint.Min30))
                {
                    ...
                    IO.ReadInc(
                      ...
                      eDef.Rplus.Min30,
                      eDef.Rminus.Min30,
                      Emera3Request.Depth30MinDefault,
                      FineTuneUtils.ReadDeepSync30Local(FineTune, this, requestParam.DeepSync, Emera3Request.Depth30MinDefault));
                ...
                if (session.LaunchPoint(info.NextPoint.Month1))
                {
                    ...
                    IO.ReadInc(
                      ...
                      Emera3Request.DepthMonthCounter,
                      requestParam.DeepSync,
                      ETariff.NoTariff);
