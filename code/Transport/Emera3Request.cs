...
using Sys.Services.Drv.Emera3.Utils;
...
namespace Sys.Services.Drv.Emera3.Transport
{
    /// <summary>Представляет реализацию всех запросов к устройству</summary>
    public class Emera3Request
    {
            ...
            out Energy response)
        {
            ...
            {
                if ((request.Index > Emera3Request.DepthDayCounter) || (request.Index > 45)) return result;
            ...
            {
                if ((request.Index > Emera3Request.DepthMonthCounter) || (request.Index > 13)) return result;
                if (tariff == ETariff.NoTariff)
                {
